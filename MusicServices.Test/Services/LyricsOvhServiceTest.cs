...
namespace MusicServices.Test.Services
{
    ...
    public class LyricsOvhServiceTest
    {
        ...
        public void LyricsOvhTest_GetLyric()
        {
            ...
            string lyric = lyricDirty.Replace(@"Paroles de la chanson Car Radio par Twenty One Pilots\r\n", "");
            string[] separators = { @"\n", " ", ",", "'" };
            List<string> words = lyric.Split(separators, int.MaxValue, System.StringSplitOptions.RemoveEmptyEntries)
                .Where(x => !string.IsNullOrWhiteSpace(x)).ToList();

            int wordCount = words.Count;

            Assert.IsTrue(response.LyricFullText == lyricDirty);
        ...
        public void LyricsOvhTest_GetLyric_FromDatabase()
        {
            ...
            string lyricDirty = @"LyricsOvhTest_GetLyric_FromDatabase unit test lyric words";
            int wordCount = 5;
            ...
            LyricsOvhService service = new LyricsOvhService();
            LyricsOvh.LyricsOvh_Request request
                = new LyricsOvh.LyricsOvh_Request { ArtistName = identifier, SongTitle = identifier };
